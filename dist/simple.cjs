...
exports.startsWithAt = exports.SimpleParser = void 0;
...
const startsWithAt = function (str, pos, sub) {
...
class SimpleParser {
  ...
  checkEnvironment(environment, job) {
    if (this.states[environment.name]) {
      //in?
      ...
      this.states[environment.name] = this.isTypeAt(job.text, job.lcv, environment.name)) {
        //beginning?
        this.content[environment.name] = '';
  ...
  isTypeAt(string, position, type) {
    ...
    this.environmentsByKey[type].sentinels.forEach(sentinel => {
      ...
      if (result) return;
      if (op[0] === string[position] && string.substring(position, position + op.length) == op) result = sentinel;
    });
    return result || false;
  }
  parse(xmlChars) {
